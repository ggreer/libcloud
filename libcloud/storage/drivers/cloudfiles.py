...
class CloudFilesStorageDriver(StorageDriver, OpenStackDriverMixin):
    ...
    def ex_set_container_meta_data(self, container, key, value):
        container_name = container.name
        headers = {key: value}
        response = self.connection.request('/%s' % (container_name),
                                           method='POST',
    ...
    def ex_set_cdn_index_page(self, container, index_page):
        return self.ex_set_container_meta_data(container,
                                               'X-Container-Meta-Web-Index',
                                               index_page)

    def ex_set_cdn_error_page(self, container, error_page):
        return self.ex_set_container_meta_data(container,
